<script lang="ts">
	...
	import { chatMessages } from '$lib/stores/chat_messages';
	import {
		chatHistory,
		filterHistory,
		chatHistorySubscription,
		loadMessages
	} from '$lib/stores/chat_history';
...
>
	...
	{#if chatHistoryKeys.length > 0}
		{#each chatHistoryKeys as message (message)}
			...
			>
				<Chat />
				<div class="flex-1 text-ellipsis max-h-5 overflow-hidden break-all relative text-white">{message}</div>
